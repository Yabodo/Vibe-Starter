        ...
        <div class="flex justify-between h-16">
          <div class="flex items-center">
            ...
            <div class="hidden md:ml-8 md:flex md:space-x-6">
              <NuxtLink 
                ...
                :class="$route.path === '/moderator' ? 
                  'border-blue-500 text-blue-600 dark:text-blue-400' : 
                  'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 dark:text-gray-400 dark:hover:text-gray-300'"
              >
                {{ t('moderator.nav.dashboard') }}
              </NuxtLink>
              
              <NuxtLink 
                to="/moderator/users"
                class="inline-flex items-center px-1 pt-1 border-b-2 text-sm font-medium transition-colors duration-200"
                :class="$route.path.startsWith('/moderator/users') ? 
                  'border-blue-500 text-blue-600 dark:text-blue-400' : 
                  'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 dark:text-gray-400 dark:hover:text-gray-300'"
              >
                {{ t('moderator.nav.users') }}
              </NuxtLink>
              
              <NuxtLink 
                to="/moderator/reports"
                class="inline-flex items-center px-1 pt-1 border-b-2 text-sm font-medium transition-colors duration-200"
                :class="$route.path.startsWith('/moderator/reports') ? 
                  'border-blue-500 text-blue-600 dark:text-blue-400' : 
                  'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 dark:text-gray-400 dark:hover:text-gray-300'"
              >
                {{ t('moderator.nav.reports') }}
                <span v-if="pendingReports > 0" 
                      class="ml-2 inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800 dark:bg-red-900/20 dark:text-red-400">
                  {{ pendingReports }}
                </span>
              </NuxtLink>
              ...
              <NuxtLink 
                ...
                class="inline-flex items-center px-1 pt-1 border-b-2 text-sm font-medium transition-colors duration-200"
                :class="$route.path.startsWith('/moderator/content') ? 
                  'border-blue-500 text-blue-600 dark:text-blue-400' : 
                  'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 dark:text-gray-400 dark:hover:text-gray-300'"
              >
                {{ t('moderator.nav.content') }}
              </NuxtLink>
            </div>
          </div>
          ...
          <div class="flex items-center space-x-4">
            ...
            <NuxtLink 
              v-if="userRole === 'ADMIN'"
              to="/admin"
              class="inline-flex items-center px-3 py-2 border border-red-300 dark:border-red-600 rounded-md text-sm font-medium text-red-700 dark:text-red-300 bg-red-50 dark:bg-red-900/20 hover:bg-red-100 dark:hover:bg-red-900/30 transition-colors duration-200"
            >
              {{ t('moderator.nav.adminPanel') }}
            </NuxtLink>
            ...
            <div class="relative">
              ...
              >
                <div class="py-1">
                  ...
                  >
                    {{ t('moderator.nav.profile') }}
                  </NuxtLink>
                  <button 
                    @click="signOut"
                    class="block w-full text-left px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700"
                  >
                    {{ t('moderator.nav.signOut') }}
                  </button>
                </div>
              </div>
            </div>
...
// User state
const userName = ref('Moderator User')
const userEmail = ref('moderator@example.com')
const userRole = ref('MODERATOR')
const userAvatar = ref(null)

// UI state
const showUserMenu = ref(false)
const showMobileMenu = ref(false)
const pendingReports = ref(8)
