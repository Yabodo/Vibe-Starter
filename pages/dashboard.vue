<template>
  <div>
    <div class="mb-8">
      <h1 class="text-3xl font-bold text-gray-800 dark:text-gray-100 mb-2 transition-colors duration-300">{{ $t('dashboard.title') }}</h1>
      <p class="text-gray-600 dark:text-gray-300 transition-colors duration-300">{{ $t('dashboard.subtitle') }}</p>
    </div>
    
    <div v-if="!loading && user" class="grid grid-cols-1 lg:grid-cols-2 gap-8">
      <!-- User Info Card -->
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 transition-colors duration-300">
        <h2 class="text-xl font-semibold mb-4 text-gray-800 dark:text-gray-100">{{ $t('dashboard.accountInfo.title') }}</h2>
        <div class="space-y-3">
          <div>
            <span class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ $t('dashboard.accountInfo.name') }}:</span>
            <p class="text-gray-800 dark:text-gray-100">{{ user.name || $t('dashboard.accountInfo.notProvided') }}</p>
          </div>
          <div>
            <span class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ $t('dashboard.accountInfo.email') }}:</span>
            <p class="text-gray-800 dark:text-gray-100">{{ user.email }}</p>
          </div>
          <div>
            <span class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ $t('dashboard.accountInfo.role') }}:</span>
            <span class="inline-block bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200 px-2 py-1 rounded text-sm">
              {{ user.role }}
            </span>
          </div>
          <div>
            <span class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ $t('dashboard.accountInfo.memberSince') }}:</span>
        ...
        <div class="mt-6">
          ...
          >
            {{ $t('dashboard.accountInfo.editProfile') }}
          </NuxtLink>
        </div>
      </div>
      ...
      <!-- Subscription Card -->
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 transition-colors duration-300">
        <h2 class="text-xl font-semibold mb-4 text-gray-800 dark:text-gray-100">{{ $t('dashboard.subscription.title') }}</h2>
        <div v-if="subscription" class="space-y-3">
          <div>
            <span class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ $t('dashboard.subscription.plan') }}:</span>
          ...
          <div>
            <span class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ $t('dashboard.subscription.status') }}:</span>
            <span 
          ...
          <div v-if="subscription.endDate">
            <span class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ $t('dashboard.subscription.expires') }}:</span>
...
const fetchUserData = async () => {
  ...
  try {
    ...
    if (err.status === 401) {
      ...
      error.value = $t('dashboard.errors.loadUserData')
    }
  } finally {
    loading.value = false
  }
