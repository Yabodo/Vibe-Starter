<template>
  ...
  >
    ...
    <div class="absolute inset-0 flex items-center justify-between px-1 pointer-events-none">
      <!-- Sun background (when dark mode is off) -->
      <svg
        class="w-3 h-3 text-yellow-500 opacity-60 transition-opacity duration-300"
        :class="isDark ? 'opacity-30' : 'opacity-60'"
        fill="currentColor"
        viewBox="0 0 20 20"
      >
        <path
      ...
      <!-- Moon background (when dark mode is on) -->
      <svg
        class="w-3 h-3 text-blue-400 opacity-60 transition-opacity duration-300"
        :class="isDark ? 'opacity-60' : 'opacity-30'"
        fill="currentColor"
        viewBox="0 0 20 20"
      >
        <path d="M17.293 13.293A8 8 0 016.707 2.707a8.001 8.001 0 1010.586 10.586z" />
